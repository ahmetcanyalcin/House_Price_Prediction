...
#Train ve test veri setleri ayni anda islenmek uzere ilerliyorum.

train = pd.read_csv("Week_09/Ders Notları/house_prices/train.csv")
test = pd.read_csv("Week_09/Ders Notları/house_prices/test.csv")
...
df = rare_encoder_(df, 0.01, cat_cols)

rare_analyser(df, "SalePrice", cat_cols)

# bilgi tasimayan veya olan frekansi 0.01'den dusuk olan veriler feature konusunda bir anlam ifade etmeyecekyir. yani bir cikarim saglamayacaktir. Bunun uzerine gereksiz bilgidir.
#Ornegin:
#PoolQC : 1
...
#gereksiz bilgilerin oldugu sutunda gezip gereksiz bilgiler drop edilir
...
submission_df.to_csv('Week_09/submission.csv', index=False)
